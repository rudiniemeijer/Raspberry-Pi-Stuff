...
try:
    ...
    with serial.Serial(port, 9600, parity=serial.PARITY_NONE, timeout=10.0) as ser:
        # 'warm up' with reading one input
        print("Serial port is open. Writing a line to activate sensor..")
        result=ser.write("xffx01x86x00x00x00x00x00x79")
        print("Writing OK. Waiting for sensor to reply..")
        time.sleep(1)
        print("Reading 9 bytes from sensor..")
        s=ser.read(9)
        print("Finished reading from sensor. Recalculating..")
        z=bytearray(s)
        # Calculate crc
        crc=crc8(s) 
        if crc != z[8]:
